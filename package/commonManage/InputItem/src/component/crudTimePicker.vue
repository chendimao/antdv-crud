...
interface EventHandlers {
  ...
  onOk?: (value: any) => void;
  onClear?: () => void;
  onPanelChange?: (value: Dayjs | null, mode: string) => void;
}
...
const eventHandlers = {
  ...
  onChange: (time: Dayjs | null, timeString: string) => {
    setTimeout(() => {
      if (props.item.$attrs?.onChange) {
        props.item.$attrs.onChange(props.item, props.formState, props.formData, time, timeString);
      }
      emit('change', props.item, time, timeString);
    }, 0);
  },
  ...
  onPanelChange: (value: Dayjs | null, mode: string) => {
    setTimeout(() => {
      if (props.item.$attrs?.onPanelChange) {
        props.item.$attrs.onPanelChange(props.item, props.formState, props.formData, value, mode);
      }
    }, 0);
  },
  onOk: (time: Dayjs | null) => {
    setTimeout(() => {
      if (props.item.$attrs?.onOk) {
        props.item.$attrs.onOk(props.item, props.formState, props.formData, time);
      }
    }, 0);
  },
  onClear: () => {
