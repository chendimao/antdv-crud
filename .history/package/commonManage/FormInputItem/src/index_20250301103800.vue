    ...
    <a-row :gutter="24">
      <template v-for="item in Data.values()">
        <a-col :span="item.span" v-if="item.type && (typeof item.show === 'function' ? item.show(formState, item, type)??true : item.show??true)">
          <a-form-item  v-bind="validateInfos[item.name]" :label="item.text" :name="item.name" :label-col="item.labelCol??{ style: { width: '90px' } }" :wrapper-col="item.wrapperCol??{style: {width: '200PX'}}"
           >
