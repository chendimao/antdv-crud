    ...
    for (const item of computedFun) {
      if (item.type == 'function' && !item.immediate) {
        ...
        item.fun(formState, Data, inputItem, value, props.type, otherData);
          ...
          if (item.type == 'hook' &&  Data.value.get(item.name) &&  Data.value.get(item.name)?.computedFun.length > 0) {
            for (const hookItem of Data.value.get(item.name)?.computedFun) {
          if (hookItem.type == 'function' && !hookItem.immediate) {
            // formState 所有表单值 Data 所有表单字段  inputItem 当前更改的表单字段 value 当前更改的表单值 type 当前表单是新增 insert 还是 修改 update
            hookItem.fun(formState, Data, inputItem, value, props.type, otherData);
          }
